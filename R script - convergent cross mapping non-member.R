...
df_nonentered <- subset(final_data, member == 0)
df_nonentered <- df_nonentered[!is.na(df_nonentered$CO2),]
...
df_nonentered <- df_nonentered[! df_nonentered$country %in% non_countries,]

output <- data.frame(c())

df_nonentered <- df_nonentered[df_nonentered$country %in% names(which(table(df_nonentered$country) >= 10)), ]

for (country1 in unique(df_nonentered$country)) {
  for (country2 in unique(df_nonentered$country)) { 
    if (country1 != country2) {
      
      #Subset dataset to countries
      A <- subset(df_nonentered, country == country1)
      B <- subset(df_nonentered, country == country2)
...
write.csv(output, file = "CCM_nonentered.csv")
